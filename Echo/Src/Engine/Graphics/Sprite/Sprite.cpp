...
Sprite::Sprite(Graphics* gfx, LPCWSTR fileName, int spriteWidth, int spriteHeight, int curFrame, int spriteScale)
{
	this->gfx = gfx;
	this->spriteWidth = spriteWidth;
	this->spriteHeight = spriteHeight;
	this->curSpriteFrame = curFrame;
	this->src.top = 0;
	this->src.left = spriteWidth * curFrame;
	this->src.right = src.left + spriteWidth;
	this->src.bottom = spriteHeight;
	this->timePassed = 0.0f;
...
void Sprite::DrawBack(D3DXVECTOR2 spritePos, D3DCOLOR spriteColor, bool bTimeToDraw)
{
	if (bTimeToDraw == true)
		--this->curSpriteFrame;
	if (this->curSpriteFrame - 1 < 0)
		this->curSpriteFrame = numSpriteFrames;
